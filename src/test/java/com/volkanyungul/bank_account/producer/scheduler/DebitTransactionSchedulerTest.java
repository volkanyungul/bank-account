...
class DebitTransactionSchedulerTest {
    ...
    void shouldGenerateDebitTransactionAndPublish() {
        ...
        DebitTransactionScheduler debitTransactionScheduler = new DebitTransactionScheduler(mockTransactionGenerator, mockProducerProperties, mockApplicationEventPublisher, scheduledExecutorService);
        // when
        debitTransactionScheduler.schedule();
        // then
        await().atMost(Duration.ofSeconds(AWAIT_DURATION)).untilAsserted(() -> {
            verify(mockTransactionGenerator, atLeastOnce()).generate(eq(DEBIT), any());
