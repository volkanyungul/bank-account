...
class DebitTransactionSchedulerTest {
    ...
    @InjectMocks
    private DebitTransactionScheduler debitTransactionScheduler;
    ...
    void setUp() {
        ...
        when(mockProducerTransactionConfig.getTransactionAmountRange()).thenReturn(new Range(-500000L, -200L));
        ...
        when(mockTransactionGenerator.generate(eq(DEBIT), any())).thenReturn(mockTransaction);
        debitTransactionScheduler = new DebitTransactionScheduler(mockTransactionGenerator, mockProducerProperties,
                mockApplicationEventPublisher, Executors.newSingleThreadScheduledExecutor());
    }

    @Test
    void shouldGenerateFiftyDebitTransactionsInTwoSecondsAndPublishAll() {
        // given
        ...
        await().atMost(Duration.ofSeconds(duration)).untilAsserted(() -> {
            verify(mockTransactionGenerator, times(expectedGeneratedTransactionCount)).generate(eq(DEBIT), any());
